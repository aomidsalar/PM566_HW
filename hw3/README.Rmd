...
The list of tokens has now changed -- the five most common stopwords now are *covid*, *19*, *patients*, *cancer*, and *prostate*. These words showed up on the previous list, but the other tokens in this list did not.
...
## Part 2
### Top 10 Bi-grams
These top bi-grams include some common stop word phrases (*of the*, *in the*, *and the*, *to the*), as well as some that relate to the healthcare field and include the top tokens that were seen above (*covid 19*, *prostate cancer*, *pre eclampsia*). Let's take out stop words and see how this list changes.
...
Compared to the previous table from question 1, the terms with the highest TF-IDF values are different, and arguably more interesting. For example, the terms with the highest TF-IDF for the search term *prostate cancer* are *prostate*, *androgen*, *psa*, *prostatectomy*, and *castration*. Some of these words (*prostate*) appear on the previous list and are more generalized to the search term, whereas the others (*androgen*, *psa*, *prostatectomy*, *castration*) are unique to this list, and provide a little more detail.
...
input %>%
  ...
  top_n(5, tf_idf) %>%
  arrange(desc(tf_idf)) %>%
  select(term, token, n, tf_idf) %>% knitr::kable(caption="5 Tokens from each Search Term with Highest TF-IDF Value")
```
